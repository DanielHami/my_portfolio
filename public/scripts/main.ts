...
function setupContactForm() {
  ...
  function showFormMessage(message: string, type: 'success' | 'error' | 'info' = 'info') {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  nodes.forEach((n, i) => {
    ...
    if (el instanceof HTMLCanvasElement && typeof Chart !== "undefined") {
          ...
          options: {
            ...
            scales: {
              ...
              y: {
                suggestedMin: 50,
                suggestedMax: 90,
                ticks: { color: 'rgba(255,255,255,0.7)', font: { size: 11 }, callback: (v: number) => v + '%' },
                grid: { color: 'rgba(255,255,255,0.06)' }
              }
            }
          }
